...
AMCLOdom::AMCLOdom() : AMCLSensor()
{
  ...
  // Custom AMCL:
  // 노면 상태에 따른 odometry noise scale.
  // 1.0 = 기존 AMCL과 동일
  this->material_scale_ = 1.0;
}

////////////////////////////////////////////////////////////////////////////////
// Custom AMCL: material scale setter
void
AMCLOdom::SetMaterialScale(double scale)
{
  // 너무 작거나 너무 큰 값 방지
  if (scale < 0.1)
    scale = 0.1;
    ...
    case ODOM_MODEL_OMNI:
    {
      ...
      for (int i = 0; i < set->sample_count; i++)
      {
        ...
        delta_trans_hat = delta_trans +
                          pf_ran_gaussian(this->material_scale_ * trans_hat_stddev);
        delta_rot_hat = delta_rot +
                        pf_ran_gaussian(this->material_scale_ * rot_hat_stddev);
        delta_strafe_hat = 0 +
                           pf_ran_gaussian(this->material_scale_ * strafe_hat_stddev);
    ...
    case ODOM_MODEL_DIFF:
    {
      ...
      for (int i = 0; i < set->sample_count; i++)
      {
        ...
        delta_rot1_hat = angle_diff(
          delta_rot1,
          pf_ran_gaussian(this->material_scale_ *
        ...
        delta_trans_hat = delta_trans -
          pf_ran_gaussian(this->material_scale_ *
                          (this->alpha3 * delta_trans * delta_trans +
    ...
    case ODOM_MODEL_DIFF_CORRECTED:
    {
      ...
      for (int i = 0; i < set->sample_count; i++)
      {
        ...
        delta_rot1_hat = angle_diff(
          delta_rot1,
          pf_ran_gaussian(this->material_scale_ *
                          sqrt(this->alpha1 * delta_rot1_noise * delta_rot1_noise +
                               this->alpha2 * delta_trans * delta_trans))
        );
