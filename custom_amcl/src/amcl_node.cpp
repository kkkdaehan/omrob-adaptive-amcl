...
class AmclNode
{
  ...
  private:
    ...
    // ------------------------------------------------------------------
    // Road-condition based trigger and PF reinitialization experiment
    // ------------------------------------------------------------------
    // /road_condition에서 현재 바닥 재질 이름을 받아 저장한다.
    ros::Subscriber road_condition_sub_;
    std::string current_road_condition_;
    std::string previous_road_condition_;

    // Road classifier output can flicker for a few frames.
    // Accept a surface change only after the same new label is observed
    // several times and enough time has passed since the last accepted change.
    std::string pending_road_condition_;
    int pending_road_condition_count_;
    int surface_change_confirm_count_;
    double surface_change_min_interval_;
    ros::Time last_surface_change_time_;

    // 노면별 covariance를 사용해 현재 추정 pose 주변으로 PF를 실제 재초기화한다.
    // 즉 /amcl_pose covariance만 바꾸는 것이 아니라 /particlecloud 자체가 퍼진다.
    ros::Time last_pf_reinit_time_;
    double concrete_pf_reinit_min_interval_;
    double granite_pf_reinit_min_interval_;
    double tile_pf_reinit_min_interval_;

    // CSV 주기 로그. trigger 순간만 기록하면 peak만 보이므로,
    // pose publish 시점에서도 일정 주기로 실제 PF covariance를 남긴다.
    double surface_cov_log_period_;
    ros::Time last_surface_cov_log_time_;

    // CSV 파일 경로
    std::ofstream slip_csv_;
    std::string slip_csv_path_;

    // 전체 trigger 횟수와 바닥 재질별 trigger 횟수
    int total_trigger_count_;
    std::map<std::string, int> surface_trigger_count_;

    // 바닥 재질별로 실제 적용할 AMCL update trigger 기준
    // launch file의 update_min_d/update_min_a를 직접 바꾸지 않고,
    // laserReceived() 내부에서 current_update_min_* 값을 사용한다.
    double current_update_min_d_;
    double current_update_min_a_;
    double surface_trigger_base_d_;
    double surface_trigger_base_a_;

    void roadConditionCallback(const std_msgs::String::ConstPtr& msg);
    void updateSurfaceBasedTriggerThreshold();
    void getSurfaceBasedReinitCovariance(double& cov_xx, double& cov_yy, double& cov_aa);
    double getSurfacePfReinitMinInterval();
    bool reinitializePfWithSurfaceCovariance(bool force, const std::string& reason);
    void getParticleCloudCovariance(double& cov_x, double& cov_y, double& cov_yaw);
    void writePfCsvLog(
      const ros::Time& stamp,
      const std::string& event,
      bool pf_reinitialized,
      double trigger_distance_cm,
...
AmclNode::AmclNode() :
        ...
        current_road_condition_("unknown"),
        previous_road_condition_("unknown"),
        pending_road_condition_(""),
        pending_road_condition_count_(0),
        surface_change_confirm_count_(3),
        surface_change_min_interval_(0.6),
        concrete_pf_reinit_min_interval_(1.2),
        granite_pf_reinit_min_interval_(1.0),
        tile_pf_reinit_min_interval_(0.8),
        surface_cov_log_period_(0.05),
        total_trigger_count_(0),
        current_update_min_d_(0.050),
        current_update_min_a_(0.070),
        surface_trigger_base_d_(0.040),
        surface_trigger_base_a_(0.060)
{
  ...
  // Surface-aware PF reinitialization settings.
  // surface_trigger_base_d/a: road_condition별 AMCL update trigger의 기준 거리/각도.
  // 위험도는 concrete < granite < tile 순서로 보고, 위험한 노면일수록 trigger 간격을 짧게 둔다.
  // 기본값: concrete=4.00cm, granite=3.20cm, tile=2.80cm.
  // PF reinit은 trigger가 발생해도 min_interval이 지나야 다시 수행된다.
  // CSV는 surface_cov_log_period 간격으로 실제 particle covariance를 계속 남긴다.
  private_nh_.param("surface_trigger_base_d", surface_trigger_base_d_, 0.040);
  if (surface_trigger_base_d_ < 0.0)
    surface_trigger_base_d_ = 0.040;

  private_nh_.param("surface_trigger_base_a", surface_trigger_base_a_, 0.060);
  if (surface_trigger_base_a_ < 0.0)
    surface_trigger_base_a_ = 0.060;

  // 노면별 PF 재초기화 최소 간격[sec].
  // AMCL trigger는 계속 기록하되, 이 시간 전에는 PF reinit만 다시 수행하지 않는다.
  private_nh_.param("concrete_pf_reinit_min_interval", concrete_pf_reinit_min_interval_, 1.2);
  private_nh_.param("granite_pf_reinit_min_interval", granite_pf_reinit_min_interval_, 1.0);
  private_nh_.param("tile_pf_reinit_min_interval", tile_pf_reinit_min_interval_, 0.8);
  if (concrete_pf_reinit_min_interval_ < 0.0) concrete_pf_reinit_min_interval_ = 0.0;
  if (granite_pf_reinit_min_interval_ < 0.0) granite_pf_reinit_min_interval_ = 0.0;
  if (tile_pf_reinit_min_interval_ < 0.0) tile_pf_reinit_min_interval_ = 0.0;

  private_nh_.param("surface_cov_log_period", surface_cov_log_period_, 0.05);
  if (surface_cov_log_period_ < 0.0)
    surface_cov_log_period_ = 0.0;

  // Road classifier flicker guard. A surface change is accepted only when
  // the same new label is observed several consecutive times.
  private_nh_.param("surface_change_confirm_count", surface_change_confirm_count_, 3);
  if (surface_change_confirm_count_ < 1)
    surface_change_confirm_count_ = 1;

  private_nh_.param("surface_change_min_interval", surface_change_min_interval_, 0.6);
  if (surface_change_min_interval_ < 0.0)
    surface_change_min_interval_ = 0.0;
  ...
  // Current surface name from road classifier node.
  road_condition_sub_ = nh_.subscribe("/road_condition", 10,
                                      &AmclNode::roadConditionCallback, this);

  // CSV file path can be changed in the launch file with private param slip_csv_path.
  private_nh_.param<std::string>(
    "slip_csv_path",
    slip_csv_path_,
    std::string("/home/omrob/slip_pf_step_log.csv")
  );

  slip_csv_.open(slip_csv_path_.c_str(), std::ios::out | std::ios::app);

  if (slip_csv_.is_open())
  {
    slip_csv_ << "time_sec,"
              << "surface,"
              << "event,"
    ...
    ROS_INFO("Slip PF step CSV logging started: %s", slip_csv_path_.c_str());
  }
  else
  {
    ROS_WARN("Failed to open slip PF CSV file: %s", slip_csv_path_.c_str());
  }
...
void
AmclNode::roadConditionCallback(const std_msgs::String::ConstPtr& msg)
{
  boost::recursive_mutex::scoped_lock cfl(configuration_mutex_);

  const std::string observed_surface = msg->data;

  // First valid surface: accept immediately.
  if (current_road_condition_ == "unknown")
  {
    previous_road_condition_ = current_road_condition_;
    current_road_condition_ = observed_surface;
    pending_road_condition_.clear();
    pending_road_condition_count_ = 0;
    last_surface_change_time_ = ros::Time::now();

    updateSurfaceBasedTriggerThreshold();
    reinitializePfWithSurfaceCovariance(true, "surface_change");
  }
  // Same as accepted surface: keep it and clear pending candidate.
  else if (observed_surface == current_road_condition_)
  {
    pending_road_condition_.clear();
    pending_road_condition_count_ = 0;
    updateSurfaceBasedTriggerThreshold();
  }
  // Different surface: accept only after consecutive confirmation.
  else
  {
    if (observed_surface == pending_road_condition_)
    {
      pending_road_condition_count_++;
    }
    else
    {
      pending_road_condition_ = observed_surface;
      pending_road_condition_count_ = 1;
    }

    const ros::Time now = ros::Time::now();
    const bool enough_count = pending_road_condition_count_ >= surface_change_confirm_count_;
    const bool enough_time = last_surface_change_time_.isZero() ||
      (now - last_surface_change_time_).toSec() >= surface_change_min_interval_;

    if (enough_count && enough_time)
    {
      previous_road_condition_ = current_road_condition_;
      current_road_condition_ = pending_road_condition_;
      pending_road_condition_.clear();
      pending_road_condition_count_ = 0;
      last_surface_change_time_ = now;

      updateSurfaceBasedTriggerThreshold();

      // Surface change is allowed to reinitialize PF once using our surface covariance.
      // It is not marked in the CSV event column; event remains reserved for AMCL trigger rows.
      const bool reinit_done = reinitializePfWithSurfaceCovariance(true, "surface_change");

      ROS_INFO(
        "[SURFACE ACCEPTED] %s -> %s | trigger=%.2f cm / %.2f deg | pf_reinit=%d | min_interval=%.2f sec",
        previous_road_condition_.c_str(),
        current_road_condition_.c_str(),
        current_update_min_d_ * 100.0,
        current_update_min_a_ * 180.0 / M_PI,
        static_cast<int>(reinit_done),
        getSurfacePfReinitMinInterval()
      );
    }
  }

  double cov_xx, cov_yy, cov_aa;
  getSurfaceBasedReinitCovariance(cov_xx, cov_yy, cov_aa);

  ROS_INFO_THROTTLE(
    1.0,
    "[SURFACE CONTROL] surface=%s | trigger=%.2f cm / %.2f deg | reinit_cov=(%.6f %.6f %.6f) | reinit_min_interval=%.2f sec | log_period=%.2f sec | pending=%s(%d/%d)",
    current_road_condition_.c_str(),
    current_update_min_d_ * 100.0,
    current_update_min_a_ * 180.0 / M_PI,
    cov_xx,
    ...
    cov_aa,
    getSurfacePfReinitMinInterval(),
    surface_cov_log_period_,
    pending_road_condition_.c_str(),
    pending_road_condition_count_,
    surface_change_confirm_count_
  );
}

void
AmclNode::updateSurfaceBasedTriggerThreshold()
{
  // Default: keep AMCL launch parameters when the surface is unknown.
  current_update_min_d_ = d_thresh_;
  current_update_min_a_ = a_thresh_;

  // ------------------------------------------------------------------
  // Trigger interval setting
  // ------------------------------------------------------------------
  // slip score 기준 위험도: concrete < granite < tile.
  // 따라서 tile이 가장 짧은 간격으로 AMCL update trigger를 만든다.
  // 기본값: base_d=0.050m
  //   concrete = 5.00 cm
  //   granite  = 3.75 cm
  //   tile     = 3.25 cm
  // ------------------------------------------------------------------
  if (current_road_condition_ == "polished_concrete_hallway")
  {
    current_update_min_d_ = surface_trigger_base_d_ * 1.00;
    current_update_min_a_ = surface_trigger_base_a_ * 1.00;
  }
  else if (current_road_condition_ == "polished_granite_bright")
  {
    current_update_min_d_ = surface_trigger_base_d_ * 0.75;
    current_update_min_a_ = surface_trigger_base_a_ * 0.80;
  }
  else if (current_road_condition_ == "porcelain_tile_dark")
  {
    current_update_min_d_ = surface_trigger_base_d_ * 0.65;
    current_update_min_a_ = surface_trigger_base_a_ * 0.70;
  }
}

void
AmclNode::getSurfaceBasedReinitCovariance(double& cov_xx, double& cov_yy, double& cov_aa)
{
  // PF 재초기화에 직접 사용할 covariance.
  // slip score 기준 위험도: concrete < granite < tile.
  // 따라서 tile이 가장 큰 covariance로 현재 pose 주변에 particle을 다시 뿌린다.
  if (current_road_condition_ == "polished_concrete_hallway")
  {
    const double xy_std = 0.20;  // m
    const double yaw_std = 8.0 * M_PI / 180.0;
    cov_xx = xy_std * xy_std;
    cov_yy = xy_std * xy_std;
    cov_aa = yaw_std * yaw_std;
  }
  else if (current_road_condition_ == "polished_granite_bright")
  {
    const double xy_std = 0.35;  // m
    const double yaw_std = 12.0 * M_PI / 180.0;
    cov_xx = xy_std * xy_std;
    cov_yy = xy_std * xy_std;
    cov_aa = yaw_std * yaw_std;
  }
  ...
  {
    const double xy_std = 0.45;  // m
    const double yaw_std = 16.0 * M_PI / 180.0;
    cov_xx = xy_std * xy_std;
    cov_yy = xy_std * xy_std;
    cov_aa = yaw_std * yaw_std;
  }
  ...
  {
    const double xy_std = 0.25;  // m
    const double yaw_std = 10.0 * M_PI / 180.0;
    cov_xx = xy_std * xy_std;
    cov_yy = xy_std * xy_std;
    cov_aa = yaw_std * yaw_std;
  }
...
double
AmclNode::getSurfacePfReinitMinInterval()
{
  if (current_road_condition_ == "polished_concrete_hallway")
    return concrete_pf_reinit_min_interval_;
  else if (current_road_condition_ == "polished_granite_bright")
    return granite_pf_reinit_min_interval_;
  else if (current_road_condition_ == "porcelain_tile_dark")
    return tile_pf_reinit_min_interval_;

  return concrete_pf_reinit_min_interval_;
}

bool
AmclNode::reinitializePfWithSurfaceCovariance(bool force, const std::string& reason)
{
  if (pf_ == NULL || !pf_init_)
  {
    return false;
  }

  const ros::Time now = ros::Time::now();
  const double min_interval = getSurfacePfReinitMinInterval();

  if (!force &&
      min_interval > 0.0 &&
      !last_pf_reinit_time_.isZero() &&
      (now - last_pf_reinit_time_).toSec() < min_interval)
  {
    return false;
  }
  ...
  pf_vector_t reinit_mean = pf_vector_zero();
  bool have_mean = false;

  // 현재 PF에서 가장 weight가 큰 cluster의 평균 pose를 재초기화 중심으로 사용한다.
  for (int hyp_count = 0; hyp_count < set->cluster_count; hyp_count++)
  {
    ...
    if (pf_get_cluster_stats(pf_, hyp_count, &weight, &pose_mean, &pose_cov))
    {
      ...
      {
        max_weight = weight;
        reinit_mean = pose_mean;
        have_mean = true;
      }
    }
  }

  // cluster stats가 아직 없으면 마지막 publish pose를 fallback으로 사용한다.
  if (!have_mean && !last_published_pose.header.frame_id.empty())
  {
    reinit_mean.v[0] = last_published_pose.pose.pose.position.x;
    reinit_mean.v[1] = last_published_pose.pose.pose.position.y;
    reinit_mean.v[2] = tf2::getYaw(last_published_pose.pose.pose.orientation);
    have_mean = true;
  }
  ...
  pf_matrix_t reinit_cov = pf_matrix_zero();
  reinit_cov.m[0][0] = cov_xx;
  reinit_cov.m[1][1] = cov_yy;
  reinit_cov.m[2][2] = cov_aa;

  pf_init(pf_, reinit_mean, reinit_cov);

  // 다음 laserReceived 흐름에서 pf_odom_pose_를 현재 odom pose로 다시 맞추고,
  // laser update를 강제로 한 번 수행하도록 만든다.
  pf_init_ = false;
  m_force_update = true;
  resample_count_ = 0;
  last_pf_reinit_time_ = now;

  ROS_INFO(
    "[PF REINIT] reason=%s | surface=%s | mean=(%.3f %.3f %.3f deg) | cov=(%.6f %.6f %.6f) | min_interval=%.2f sec",
    reason.c_str(),
    current_road_condition_.c_str(),
    reinit_mean.v[0],
    reinit_mean.v[1],
    reinit_mean.v[2] * 180.0 / M_PI,
    cov_xx,
    cov_yy,
    cov_aa,
    min_interval
...
void
AmclNode::getParticleCloudCovariance(double& cov_x, double& cov_y, double& cov_yaw)
{
  cov_x = -1.0;
  cov_y = -1.0;
  cov_yaw = -1.0;
  ...
  pf_sample_set_t* set = pf_->sets + pf_->current_set;
  if (set == NULL || set->sample_count <= 0)
    return;

  const int n = set->sample_count;
  ...
  // Particle cloud의 실제 퍼짐을 보기 위해 sample들을 동일 가중치로 계산한다.
  // sensor update 이후 weight 변화 자체는 resampling에서 sample 분포 변화로 반영된다.
  for (int i = 0; i < n; i++)
  {
    mean_x += set->samples[i].pose.v[0];
    mean_y += set->samples[i].pose.v[1];
    mean_sin += sin(set->samples[i].pose.v[2]);
    mean_cos += cos(set->samples[i].pose.v[2]);
  }
  ...
  double mean_yaw = atan2(mean_sin, mean_cos);
  ...
  for (int i = 0; i < n; i++)
  {
    double dx = set->samples[i].pose.v[0] - mean_x;
    double dy = set->samples[i].pose.v[1] - mean_y;
    double dyaw = angle_diff(set->samples[i].pose.v[2], mean_yaw);
...
)
{
  ...
  int current_surface_count = 0;
  if (surface_trigger_count_.find(current_road_condition_) != surface_trigger_count_.end())
  {
    current_surface_count = surface_trigger_count_[current_road_condition_];
  }

  slip_csv_ << std::fixed << std::setprecision(6)
            << stamp.toSec() << ","
            << current_road_condition_ << ","
            << event << ","
            << static_cast<int>(pf_reinitialized) << ","
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  bool update = false;
  double trigger_distance_cm = 0.0;
  double trigger_angle_deg = 0.0;
  bool pf_reinit_done_this_scan = false;
  ...
  if(pf_init_)
  {
    ...
    // ------------------------------------------------------------------
    // Surface-aware AMCL update trigger
    // ------------------------------------------------------------------
    // 기존 AMCL은 x, y, yaw를 각각 threshold와 비교한다.
    // 여기서는 직선 이동량 sqrt(dx^2 + dy^2)와 yaw 변화량을 사용해서
    // 실제 몇 cm / 몇 deg마다 update가 발생했는지 CSV로 보기 쉽게 만든다.
    updateSurfaceBasedTriggerThreshold();

    double d_trans = sqrt(delta.v[0] * delta.v[0] + delta.v[1] * delta.v[1]);
    double d_yaw = fabs(delta.v[2]);

    trigger_distance_cm = d_trans * 100.0;
    trigger_angle_deg = d_yaw * 180.0 / M_PI;

    update = d_trans > current_update_min_d_ ||
             d_yaw > current_update_min_a_;
    ...
    {
      total_trigger_count_++;
      surface_trigger_count_[current_road_condition_]++;

      // Trigger 발생 시 현재 pose 주변으로 PF 재초기화를 시도한다.
      // 단, 노면별 min_interval이 지나지 않았으면 trigger만 기록하고 reinit은 생략한다.
      pf_reinit_done_this_scan = reinitializePfWithSurfaceCovariance(false, "trigger");

      double cov_xx, cov_yy, cov_aa;
      getSurfaceBasedReinitCovariance(cov_xx, cov_yy, cov_aa);

      ROS_INFO(
        "[AMCL TRIGGER] surface=%s | all=%d | surface=%d | motion=%.2f cm | rotation=%.2f deg | threshold=%.2f cm / %.2f deg | pf_reinit=%d | reinit_cov=(%.6f %.6f %.6f)",
        current_road_condition_.c_str(),
        total_trigger_count_,
        surface_trigger_count_[current_road_condition_],
        trigger_distance_cm,
        ...
        current_update_min_d_ * 100.0,
        current_update_min_a_ * 180.0 / M_PI,
        static_cast<int>(pf_reinit_done_this_scan),
        cov_xx,
        cov_yy,
        cov_aa
      );
  ...
  // Periodic CSV rows between AMCL triggers. These rows have an empty
  // event field so that trigger rows remain easy to find, while cov decay
  // can still be inspected between triggers.
  if (pf_init_ && !update)
  {
    const ros::Time now_for_log = ros::Time::now();
    if (surface_cov_log_period_ == 0.0 ||
        last_surface_cov_log_time_.isZero() ||
        (now_for_log - last_surface_cov_log_time_).toSec() >= surface_cov_log_period_)
    {
      double pf_cov_x = -1.0;
      double pf_cov_y = -1.0;
      double pf_cov_yaw = -1.0;
      getParticleCloudCovariance(pf_cov_x, pf_cov_y, pf_cov_yaw);

      last_surface_cov_log_time_ = now_for_log;
      writePfCsvLog(
  ...
  else if(pf_init_ && lasers_update_[laser_index])
  {
    ...
    // 이 직후가 motion model noise에 의해 particle이 퍼진 순간이다.
  ...
  if(lasers_update_[laser_index])
  {
    ...
    // Laser likelihood update. 이 단계에서 각 particle의 weight가 갱신된다.
    ...
    {
      // Resampling. weight가 높은 particle 주변으로 sample이 다시 뽑히므로
      // 이 직후 covariance가 줄어드는지 확인할 수 있다.
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      // ------------------------------------------------------------------
      // Custom AMCL: PF reinitialization result logging
      // ------------------------------------------------------------------
      // p.pose.covariance는 더 이상 publish 직전에 가짜로 덮어쓰지 않는다.
      // PF 재초기화 때 사용한 노면별 covariance가 particle set에 직접 반영되고,
      // 여기서는 AMCL/PF가 실제 계산한 covariance만 기록/발행한다.
      // ------------------------------------------------------------------
      const double pf_cov_x = p.pose.covariance[6 * 0 + 0];
      const double pf_cov_y = p.pose.covariance[6 * 1 + 1];
      const double pf_cov_yaw = p.pose.covariance[6 * 5 + 5];

      const ros::Time now_for_log = ros::Time::now();
      const bool csv_log_due = surface_cov_log_period_ == 0.0 ||
          last_surface_cov_log_time_.isZero() ||
          (now_for_log - last_surface_cov_log_time_).toSec() >= surface_cov_log_period_;

      if (update || csv_log_due)
      {
        last_surface_cov_log_time_ = now_for_log;
        writePfCsvLog(
          laser_scan->header.stamp,
          update ? "trigger" : "",
          pf_reinit_done_this_scan,
          trigger_distance_cm,
          trigger_angle_deg,
          pf_cov_x,
          pf_cov_y,
      ...
      ROS_INFO_THROTTLE(
        1.0,
        "[AMCL PF COV] surface=%s | pf_cov=(%.6f %.6f %.6f)",
        current_road_condition_.c_str(),
        pf_cov_x,
        pf_cov_y,
        pf_cov_yaw
